...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
	 ...
	 * The axe angle
	 */
    private float mTheta = (float)(6.0f*(Math.PI/180.0));	
	...
	public Carousel(Context context, AttributeSet attrs, int defStyle) {
		...
		// Retrieve settings
		TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel);
		mAnimationDuration = arr.getInteger(R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel_android_animationDuration, 400);
		mUseReflection = arr.getBoolean(R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel_UseReflection, false);	
		int selectedItem = arr.getInteger(R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel_SelectedItem, 0);
		int imageArrayID = arr.getResourceId(R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel_Items, -1);		
		TypedArray images = getResources().obtainTypedArray(imageArrayID);
		
		int min = arr.getInteger(R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel_minQuantity, MIN_QUANTITY);
		int max = arr.getInteger(R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel_maxQuantity, MAX_QUANTITY);
		
		float mTheta = arr.getFloat(R.styleable.com_zia_freshdocs_widget_fileexplorer_Carousel_maxTheta, MAX_THETA);
    ...
    protected int getChildDrawingOrder(int childCount, int i) {
    	...
    	ArrayList<CarouselFrame> sl = new ArrayList<CarouselFrame>();
    	for(int j = 0; j < childCount; j++)
    	{
    		CarouselFrame view = (CarouselFrame)getAdapter().getView(j,null, null);
    		if(i == 0)
    			view.setDrawn(false);
    		sl.add((CarouselFrame)getAdapter().getView(j,null, null));
    	}
	...
	protected boolean getChildStaticTransformation(View child, Transformation transformation) {
		...
		mCamera.translate(((CarouselFrame)child).getX(), ((CarouselFrame)child).getY(), 
							((CarouselFrame)child).getZ());
	...
	private class ImageAdapter extends BaseAdapter {
		...
		public void SetImages(TypedArray array, boolean reflected){
			// rectangle between image and reflected
			final int reflectionGap = 0;
						
			Drawable[] drawables = new Drawable[array.length()];
			mImages = new CarouselFrame[array.length()];
...
//							0x70ffffff, 0x00ffffff, TileMode.CLAMP);
				...
				CarouselFrame imageView = new CarouselFrame(mContext);
...
//				imageView.setScaleType(ScaleType.MATRIX);
    ...
    private void Calculate3DPosition(CarouselFrame child, int diameter, float angleOffset){
...
//        int childTop = -50;
    ...
    private void makeAndAddView(int position, float angleOffset) {
        ...
        if (!mDataChanged) {
            child = (CarouselFrame)mRecycler.get(position);
            if (child != null) {
            ...
            else
            {
                ...
                child = (CarouselFrame)mAdapter.getView(position, null, this);
    ...
    private void scrollIntoSlots(){
    	...
    	ArrayList<CarouselFrame> arr = new ArrayList<CarouselFrame>();
    	
        for(int i = 0; i < getAdapter().getCount(); i++)
        	arr.add(((CarouselFrame)getAdapter().getView(i, null, null)));
        
        Collections.sort(arr, new Comparator<CarouselFrame>(){
			@Override
			public int compare(CarouselFrame c1, CarouselFrame c2) {
	...
	void scrollToChild(int i){		
		...
		CarouselFrame view = (CarouselFrame)getAdapter().getView(i, null, null);
     ...
     */
    private void setUpChild(CarouselFrame child, int index, float angleOffset) {
        ...
        if(mInLayout)
        {
	        h = 7*(getMeasuredHeight() - getPaddingBottom() - getPaddingTop())/10;
    ...
    void trackMotionScroll(float deltaAngle) {
        ...
        for(int i = 0; i < getAdapter().getCount(); i++){
        	CarouselFrame child = (CarouselFrame)getAdapter().getView(i, null, null);
